...
export class CalDavProvider extends CalDavTransport {
  ...
  async putEventWithUid(
    ...
    let etag: string;
    try {
      // Do not pass `headers` here — tsdav's defaultParam replaces (not merges)
      // headers, which drops the Basic auth token. tsdav already sends
      // If-None-Match: * in createCalendarObject.
